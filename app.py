...
if "layout_mode" not in st.session_state:
    q_layout = st.query_params.get("layout", "compact")
    st.session_state["layout_mode"] = "Classic" if "classic" in q_layout else "Compact"

if "color_scheme" not in st.session_state:
    q_color = st.query_params.get("color", "uniform")
    ...
    with st.container(border=True):
        ...
        with c_set1:
            new_layout = st.radio(
                "Layout Mode",
                ["Compact", "Classic"],
                index=0 if st.session_state["layout_mode"] == "Compact" else 1
            )
        with c_set2:
            new_color = st.radio(
                "Pie Chart Color Scheme",
                ["Uniform", "Custom colours"],
                index=0 if st.session_state["color_scheme"] == "Uniform" else 1
            )
